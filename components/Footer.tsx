...
const Footer: React.FC<FooterProps> = ({ content }) => {
    ...
    return (
        <footer className="text-white bg-[linear-gradient(180deg,#0f1f10_0%,#0a150a_100%)]">
            <div className="container py-16 md:py-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-12">
                    ...
                    <div className="col-span-2 md:col-span-1 space-y-4">
                        ...
                        <p className="text-sm leading-relaxed max-w-52 text-white/50">
                            ...
                            <div className="flex flex-wrap gap-2.5">
                                {socialLinks.map(s => (
                                    <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer"
                                        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 hover:scale-110 bg-white/8 text-white/50 ${s.color}`}
                                        title={s.name}>
